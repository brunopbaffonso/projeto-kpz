				...
				<fieldset>
					...
					@if ($totalRows>0)
						<span class="total-info pull-right">
							{{trans_choice('Página :current_page  de :total_pages. Total de :total_rows Registros.', $totalRows, [
		...
		<div class="search simple-search {{isset($searchedValue) && $searchedValue<>'' ? 'searched' : ''}}">		
			<form action="{{$url}}" method="get">			
				@foreach($fieldsRequest as $field=>$valor)
					...
					@endif
				@endforeach
		      	<input type="text" name="search" class="form-control input-search" placeholder="@lang('Digite aqui')" value="{{$searchedValue}}">
	...
	<div class="row">
		...
		<div class="col-md-4">
			<div class="showing-rows-info pull-right">
				...
				<span>@lang('registros por página')</span>
			</div>
		</div>
	</div>	
		...
		<div class="col-md-{!! $totalPages>1 ? '9' : '12' !!}">	
			...
			<div class="input-group">				
				<select name="export" class="form-control">
					<option value="">@lang('Selecione opção para exportar')</option>
				...
				<a href="#" data-href="{{$urlExport}}" target="_blank" class="input-group-addon btn-export" title="@lang('Simplegrid::grid.Export')" data-alert-msg="@lang('Simplegrid::grid.Select a format for export!')">
					<span class="glyphicon glyphicon-download"></span> @lang('Exportar')
				</a>
			</div>		
			@endif
		</div>
